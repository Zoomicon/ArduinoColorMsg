 ...
 * ColorTransmit 2.0
 * 
 * Read an emitted color value and display Red, Green and Blue elements of it (via LED, LCD, Serial and Buzzer)
 * 
 * Components:
 *   Arduino board
...
void setup() {
  ...
  show_message_serial("ColorTransmit 2.0");
  show_message_lcd("ColorTransmit 2.0", 2000); //show version for 2sec (2000msec)
...
void loop() {
  ...
  show_color_name_lcd(1, isRed, isGreen, isBlue); //print color name (for each detected color element) to given LCD row
